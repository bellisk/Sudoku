...
def solve(grid):
    ...
    while made_progress and not solved_grid(grid):
        ...
        for shape in get_shapes(grid):
            ...
            two_number_cells = []
            for cell in shape:
                if type(cell) == list and len(cell) == 2:
                    two_number_cells.append(cell)
            for c in two_number_cells:
                two_number_cells2 = copy.copy(two_number_cells)
                two_number_cells2.remove(c)
